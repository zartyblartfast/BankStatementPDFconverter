"""Generate an Expenditure by Category HTML report.

Usage:
  python -m scripts.report_expenditure_by_category
...
def _build_html(txns: list[dict], all_months: list[str]) -> str:
    """Build the Expenditure by Category HTML report."""
...
<head>
<meta charset="utf-8">
<title>Expenditure by Category</title>
...
<style>
  ...
  .summary-card .total {{ font-size:1.8rem; font-weight:700; margin-bottom:12px; color:#ef4444; }}
...
const COLOURS = [
  ...
  '#64748b','#a855f7','#14b8a6','#f43f5e','#6366f1','#84cc16','#d946ef',
  '#fb923c','#facc15','#c084fc','#f472b6','#22d3ee','#94a3b8',
];
...
function render() {{
  ...
  sorted.forEach(([cat, amt], i) => {{
    ...
    tr.innerHTML =
      `<td><span class="cat-bar" style="width:${{barWidth}}%;max-width:120px;background:${{colour}}"></span>${{esc(cat)}}</td>` +
      `<td>\\u00a3${{fmt(amt)}}</td>` +
      `<td>${{pct.toFixed(1)}}%</td>`;
...
def main():
    ...
    filepath = REPORTS_DIR / "expenditure_by_category.html"
